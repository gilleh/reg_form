...
class Reg extends PDO {
	...
	public function DB_reg_submit($data){
		try {
			...
			if ($query->rowCount() === 1){
				setcookie("first_name", $data['first_name'], time()+3600, "/reg_form/");
				setcookie("last_name", $data['last_name'], time()+3600, "/reg_form/");
				setcookie("date_of_birth", $data['date_of_birth'], time()+3600, "/reg_form/");
			}
